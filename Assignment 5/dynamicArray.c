...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if(right < last)        //This means there are two children
    {
    ...
    else if(left < last)    //Only one child
    {
...
void buildHeap(DynamicArray* heap, compareFunction compare)
{
    assert(heap);
    // FIXME: implement
    int size;			//Initialize the values for the heap
    int i;
    size = heap -> size;

    for(i = (size / 2) - 1; i >= 0; i--)
    {
        adjustHeap(heap, size, i, compare);	//Make sure values are in the proper place
    }
}
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
    ...
    adjustHeap(heap, dySize(heap), 0, compare);			//Adjust the heap to get values in proper place
}
...
TYPE dyHeapGetMin(DynamicArray* heap)
{
    assert(heap);
    // FIXME: implement
    return heap -> data[0];			//First element will be the min
}
